...
export function Settings({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const handleApiKeyDown = (e: React.KeyboardEvent) => e.key === 'Enter' && handleSaveClicked();
    ...
    <Modal open={open} onClose={onClose}>
      <ModalDialog sx={{ minWidth: '40vw' }}>
        ...
        <Box sx={{ mt: 3, minWidth: 300 }}>
          <Typography sx={{ mb: 1 }}>输入 OpenAI API Key</Typography>

          <Input variant="outlined" placeholder={'sk-...'} error={!isValidKey} value={apiKey} onChange={handleApiKeyChange} onKeyDown={handleApiKeyDown} />

          <Typography sx={{ mt: 3, mb: 1 }}>选择大语言模型</Typography>

          <Select variant="outlined" value={gptModel} onChange={handleGptModelChange}>
